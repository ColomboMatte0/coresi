...
#         dataset = f["listmode"]
#         for line_n, row in enumerate(dataset):
#             if line_n >= start_position:
#                 try:
#                     event = Event(line_n, row, E0, volume_center, volume_dim, tol=tol, format="HDF5")
#                     events.append(event)
#                 except ValueError as e:
#                     skipped_events += 1
#                     logger.debug(f"Skipping event {row} REASON: {e}")
#                     continue
...
#     return Events(str(file_name), n_events, cameras[0].device, E0)
